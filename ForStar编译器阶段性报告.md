...
文件目录结构如下：
- AST: ast文件夹
     - Builder： 构造ast的各种方法，例如将symbol替换为ast中的symbol或者其子树中的一个node的label或者其值等等。
     - Emitter：将中间代码的结果以.ll文件的形式输出。
     - Scope： 存储当前每一层代码中的变量信息。
     - Visitor：遍历语法树，根据文法逻辑和中间代码形式，将语法树翻译成所需要的中间代码。
